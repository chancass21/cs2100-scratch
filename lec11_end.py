'''
    ...
    In class, we...
    - improved the bar plot (customizations, xticks, legend)
    - wrote the three versions of the plot function we're calling

    Look in main() for some specific pandas things, not related to plotting
    per se, but might come in handy for upcoming lab/HW:
    * filter a DF using & to look for something like "col 1 has this AND col 2 has that", gives you
      rows that have both
    * sort_values(), we also saw in class on 2/2/26
    * df[col].astype(str), treat a DF column as a string
    * df[col].tolist(), turn a DF column into a python list
    * np.arange(num), create a numpy array from 0 to num. An array is like a list, but faster.
...
def plot_hockey_v2(y_values: pd.Series, x_values: Optional[pd.Series] = None,
                   plot_func: Callable[..., Any] = plt.plot):
    '''
        ...
        Params:
            y_values (pd.Series), the y values to plot
            x_values (pd.Series), the x values to plot - Optional, defaults to None.
            plot_func (Callable[..., Any]), the matplotlib function to use for plotting.
                defaults to plt.plot
...
def plot_hockey_v3(y_values: pd.Series, x_values: Optional[pd.Series] = None,
                   plot_func: Callable[..., Any] = plt.plot, **kwargs):
    '''
        Call the given plotting function and plot the values. 
        In this version, our function takes in both x_values (opt) and y_values (req)
