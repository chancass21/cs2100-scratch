...
def read_mileage_input(filename: str) -> dict[str, float]:
    '''
        read user mileage data from a file, and create and retuern
        a dictionary where keys = dates and values = miles
        Expected file format - exactly two lines, comma separated values
            date,date,date
            mile,mile,mile
        ...
        Raises:
            IOError if reading either line of file returns None/empty
...
def main() -> None:
    ''' create dictionaries for Laney and Nate's last week of running and compute stats about them '''
    laney_miles = read_mileage_input(LANEYFILE)
    nate_miles = read_mileage_input(NATEFILE)
    ...
    if date not in laney_miles or date not in nate_miles:
