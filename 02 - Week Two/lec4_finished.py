...
def generate_mileage_stats(miles: Optional[dict[str, float]]) -> Optional[dict[str, float]]:
    ''' compute basic stats from a mileage list: total, avg daily
        return a dictionary with stat : value pairs, or None
  
       Parameters:
           miles (list[float]): a list containing a runner's mileage per-day
      
       Returns:
           optional dict[str, float]: summary stats for total miles 
                    and avg daily miles for that runner
                    or None if given dictionary is empty or None
...
def print_summary(name: str, stats: Optional[dict[str, float]]) -> None:
